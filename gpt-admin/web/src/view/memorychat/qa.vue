    ...
    <div class="gva-table-box">
      ...
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="80px">
        <el-form-item label="chatId:" prop="chatid">
          <el-input v-model="formData.chatId" disabled />
        </el-form-item>
        ...
        <el-form-item label="Messages:" prop="title">
          <div style="width: 100%; padding: 20px 20px 20px 20px; border-radius: 6px; border: 1px solid rgba(0, 0, 0, 0.15);" v-html="formData.allmessages" >
          </div>
        </el-form-item>
...
import { memerychat_raw } from '../../api/qa'
function getUuid() {
...
const btnSubmit = async() => {
  ...
  formData.value.allmessages +=  '<strong>Question:</strong>:</br>'
  formData.value.allmessages += formData.value.question + '</br>'
  // formData.value.allmessages += '<strong>query</strong>:</br>'
  // formData.value.allmessages += '  12345</br>'
  // formData.value.allmessages += 'answer==>:</br><hr />'

  loading.value = true
  const ret = await memerychat_raw(formData.value)
  loading.value = false
  // formData.value.raw_gpt_response = JSON.stringify(ret.data.raw_gpt_response)
  // formData.value.answer = ret.data.answer
  formData.value.allmessages += '<strong>SDK Assistant:</strong>: </ br>'
  formData.value.allmessages += ret.data.answer.replace(/\n/g, '<br>') + '</br>'
  formData.value.allmessages += '<hr style="color:red"/>'
